-- 🔧 P0 FIX: Corrigir verificação de profile ativo
-- A tabela profiles NÃO tem coluna "status", mas tem "is_banned"
-- Corrigir a função validate_session_epoch
...
BEGIN
  ...
  -- 7. Profile NOT banned (usando is_banned ao invés de status)
  SELECT COALESCE(p.is_banned, false) INTO v_profile_banned
