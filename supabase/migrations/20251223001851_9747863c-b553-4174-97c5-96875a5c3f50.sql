...
BEGIN
    ...
    IF v_is_immune THEN
        ...
        VALUES (v_session.user_id, v_session.id, 'violation', v_session.lesson_id, v_session.provider_video_id, p_ip_address, p_user_agent,
    ...
    INSERT INTO public.video_violations (
        ...
        p_details, p_key_pressed, v_action_taken, p_ip_address, p_user_agent
    );
...
DROP POLICY IF EXISTS "val_user_insert" ON public.video_access_logs;
...
CREATE POLICY "val_user_insert" ON public.video_access_logs FOR INSERT WITH CHECK (auth.uid() = user_id);
...
GRANT EXECUTE ON FUNCTION public.create_video_session(UUID, UUID, UUID, TEXT, TEXT, INET, TEXT, TEXT, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION public.video_session_heartbeat(TEXT, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION public.register_video_violation(TEXT, video_violation_type, INTEGER, JSONB, TEXT, INET, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_domain_authorized(TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.end_video_session(TEXT, INTEGER, DECIMAL) TO authenticated;
