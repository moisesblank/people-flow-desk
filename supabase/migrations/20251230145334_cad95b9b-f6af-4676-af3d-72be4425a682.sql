-- Fix fn_get_book_for_reader: digest is in extensions schema (avoid pgcrypto schema reference)
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'watermark', jsonb_build_object(
      ...
      'userCpf', v_user_cpf,
      'userId', v_user_id,
      'seed', encode(extensions.digest(v_user_id::text || v_book.id::text || now()::text, 'sha256'), 'hex')
    ),
    'isOwner', v_is_owner,
