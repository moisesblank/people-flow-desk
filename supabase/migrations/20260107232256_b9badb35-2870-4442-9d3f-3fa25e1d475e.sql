-- Remover as duas versões duplicadas da função create_video_session
DROP FUNCTION IF EXISTS public.create_video_session(uuid, uuid, uuid, text, text, integer, text, text, text);
DROP FUNCTION IF EXISTS public.create_video_session(uuid, uuid, uuid, text, text, inet, text, text, integer);

-- Recriar uma versão única e consistente (com p_ip_address como TEXT para simplicidade)
...
BEGIN
  ...
  SELECT 
    COALESCE(p.full_name, p.nome, split_part(au.email, '@', 1)),
    p.cpf
