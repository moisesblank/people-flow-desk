-- CORREÇÃO CRÍTICA: finish_simulado_attempt estava usando tabela inexistente simulado_questions
-- A estrutura real usa question_ids (UUID[]) diretamente na tabela simulados
...
BEGIN
  ...
  -- CORREÇÃO: Contar total de questões usando array_length em question_ids (não tabela simulado_questions)
  v_total_questions := COALESCE(array_length(v_simulado.question_ids, 1), 0);
  ...
  IF v_attempt.is_scored_for_ranking = true THEN
    ...
    PERFORM public.add_user_xp(
      ...
      'Simulado: ' || v_simulado.title  -- description
    );
  END IF;
