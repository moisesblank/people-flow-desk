...
-- Corrigir trigger com valores corretos de prioridade
...
BEGIN
  ...
  -- Se status mudou para PAGO, completar tarefa do calendário
  ...
  IF NEW.status_pagamento IN ('pendente', 'atrasado') THEN
    ...
    IF v_task_id IS NOT NULL THEN
      ...
      SET 
        ...
        priority = CASE WHEN NEW.status_pagamento = 'atrasado' THEN 'high' ELSE 'medium' END,
        updated_at = now()
      WHERE id = v_task_id;
    ELSE
      ...
      SELECT
        ...
        v_task_date,
        'financeiro',
        CASE WHEN NEW.status_pagamento = 'atrasado' THEN 'high' ELSE 'medium' END,
        ur.user_id,
        true
