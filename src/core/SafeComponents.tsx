...
interface SafeDownloadProps {
  ...
  children: React.ReactNode;
  className?: string;
  onDownload?: (fileId: string) => Promise<string>; // Retorna signed URL
}
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <span className={cn("cursor-not-allowed opacity-70", className)}>
            {children}
            <Clock className="inline-block ml-1 h-3 w-3 text-amber-500" />
          </span>
        </TooltipTrigger>
    ...
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <span className={cn("cursor-not-allowed opacity-50", className)}>
            {children}
            <Lock className="inline-block ml-1 h-3 w-3 text-gray-500" />
          </span>
        </TooltipTrigger>
...
export const SafeNavItem = memo<SafeNavItemProps>(({
  ...
  return (
    ...
    >
      ...
      {!hasAccess && <Lock className="h-3 w-3 text-gray-500" />}
      {badge && (
