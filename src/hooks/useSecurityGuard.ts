...
export function useSecurityGuard() {
  ...
  const checkRateLimit = useCallback(async (
    ...
    try {
      // Rate limit local baseado em localStorage (fallback robusto)
      const storageKey = `rl_${cacheKey}`;
      const now = Date.now();
      const stored = localStorage.getItem(storageKey);
      let requests: number[] = stored ? JSON.parse(stored) : [];
      
      // Limpar requests expiradas
      requests = requests.filter(t => now - t < config.windowMs);
      
      const blocked = requests.length >= config.maxRequests;
      ...
      const result: RateLimitResult = {
        ...
        blocked: blocked,
        remaining: remaining,
        blockedUntil: blocked ? new Date(now + (config.blockMs || 60000)) : undefined,
        retryAfterSeconds: blocked ? Math.ceil((config.blockMs || 60000) / 1000) : undefined,
      };
  ...
  const logSecurityEvent = useCallback(async (
    ...
    try {
      // Log via activity_log que existe no schema
      const insertData = {
      ...
      await supabase.from('activity_log').insert(insertData as any);
    } catch (err) {
  ...
  const logAudit = useCallback(async (
    ...
    try {
      // Usar activity_log que existe no schema
      const insertData = {
        action: `audit:${category}:${action}`,
  ...
  const logContentAccess = useCallback(async (
    ...
    try {
      // Usar activity_log para registrar acesso a conteúdo
      const insertData = {
  ...
  }, [user?.id, user?.email, logSecurityEvent]);
  ...
  const canAccessUrl = useCallback(async (url: string): Promise<boolean> => {
    ...
    try {
      // Tentar usar RPC, com fallback para verificação local
      ...
      if (error) {
        // Fallback: permitir (a verificação real é feita pelo RoleProtectedRoute)
        console.warn('[Security] Access check RPC not available');
        return true;
      }
      ...
      return true; // Permitir, verificação real é feita pelo RoleProtectedRoute
    }
...
export function useContentProtection(
  ...
  // Tipo para proteção de screenshot (apenas video e pdf)
  const protectedType = contentType === 'video' || contentType === 'pdf' ? contentType : 'pdf';
  ...
  useEffect(() => {
    ...
    // Habilitar proteção de screenshot apenas para video/pdf
    const cleanup = enableScreenshotProtection(protectedType, contentId);
  ...
  }, [contentType, contentId, protectedType, logContentAccess, enableScreenshotProtection]);
