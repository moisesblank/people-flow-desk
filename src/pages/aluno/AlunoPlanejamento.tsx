...
function VideoPlayer({
  ...
  // URLs para os botões das keys
  const hubKeyUrls: Record<string, { label: string; url: string; color: string }> = {
    cronograma: { label: "Cronograma", url: "/alunos/cronograma", color: "from-blue-500 to-indigo-600" },
    tutoria: { label: "Tutoria", url: "/alunos/forum", color: "from-emerald-500 to-teal-600" },
    questoes: { label: "Questões", url: "/alunos/questoes", color: "from-amber-500 to-orange-600" },
    simulados: { label: "Simulados", url: "/alunos/simulados", color: "from-purple-500 to-violet-600" },
    materiais: { label: "Materiais", url: "/alunos/materiais", color: "from-pink-500 to-rose-600" },
    livros: { label: "Livros", url: "/alunos/livros", color: "from-orange-500 to-red-600" },
  };
...
// ============================================
// ============================================
// 🎯 UNIFIED STUDY HUB - 6 KEYS ONLY
// Year 2300 Premium Aesthetic - APAGADO header, tabela, TRAMON
// ============================================
function UnifiedStudyHub({
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden group mt-4 mb-6">
      ...
      {/* Main container */}
      <div className="relative bg-gradient-to-b from-card via-card/98 to-card/95 rounded-2xl p-4">
        {/* Top accent line */}
        <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-primary via-holo-purple to-holo-cyan" />
        
        {/* 🎯 6 PREMIUM KEYS — Year 2300 Cinematic */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          {HUB_AREAS.map((area, index) => {
            ...
            const hueColors = {
              cronograma: { from: "from-blue-500", to: "to-indigo-600", shadow: "shadow-blue-500/30", text: "text-blue-400" },
              tutoria: { from: "from-emerald-500", to: "to-teal-600", shadow: "shadow-emerald-500/30", text: "text-emerald-400" },
              questoes: { from: "from-amber-500", to: "to-orange-600", shadow: "shadow-amber-500/30", text: "text-amber-400" },
              simulados: { from: "from-purple-500", to: "to-violet-600", shadow: "shadow-purple-500/30", text: "text-purple-400" },
              materiais: { from: "from-pink-500", to: "to-rose-600", shadow: "shadow-pink-500/30", text: "text-pink-400" },
              "livros-web": { from: "from-orange-500", to: "to-red-600", shadow: "shadow-orange-500/30", text: "text-orange-400" },
            };
            
            const colors = hueColors[area.key] || hueColors.cronograma;
            ...
            return (
              <button
                ...
                className={cn(
                  "relative flex flex-col items-center justify-center gap-2 p-4 rounded-xl transition-all duration-300 border group/key",
                  "transform-gpu hover:scale-105 hover:-translate-y-1",
                  isActive
                    ? `bg-gradient-to-br ${colors.from}/20 ${colors.to}/10 border-white/30 shadow-[0_0_30px_rgba(255,255,255,0.2)]`
                    : "bg-gradient-to-br from-white/5 to-white/[0.02] border-white/10 hover:border-white/25 hover:bg-white/10"
                )}
                style={{
              ...
              >
                ...
                <div 
                  className={cn(
                    "absolute inset-0 rounded-xl opacity-0 group-hover/key:opacity-100 transition-opacity duration-500 blur-xl",
                    `bg-gradient-to-br ${colors.from}/30 ${colors.to}/20`
                  )}
                />
                
                {/* Icon container - Premium */}
                <div className={cn(
                  "relative p-3 rounded-xl transition-all duration-300 shadow-lg",
                  isActive 
                    ? `bg-gradient-to-br ${colors.from} ${colors.to} ${colors.shadow} shadow-[0_0_25px_currentColor]` 
                    : `bg-gradient-to-br ${colors.from}/20 ${colors.to}/10 group-hover/key:${colors.from}/30`
                )}>
                  <Icon className={cn(
                    "h-6 w-6 transition-all duration-300",
                    isActive 
                      ? "text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]" 
                      : `${colors.text} group-hover/key:text-white`
                  )} />
                ...
                {/* Label */}
                <span className={cn(
                  "text-xs font-bold text-center leading-tight transition-colors duration-300 tracking-wide",
                  isActive ? "text-white" : "text-white/70 group-hover/key:text-white"
                )}>
                  {area.label}
                </span>
                ...
                {area.badge && (
                  <div 
                    className={cn(
                      "absolute -top-1.5 -right-1.5 px-1.5 py-0.5 rounded-md text-[9px] font-black text-white shadow-lg",
                      "ring-2 ring-card/50",
                      area.badgeType === "ai" && "bg-gradient-to-r from-emerald-500 to-teal-500 shadow-emerald-500/50",
                      area.badgeType === "xp" && "bg-gradient-to-r from-amber-500 to-orange-500 shadow-amber-500/50",
                      area.badgeType === "treino" && "bg-gradient-to-r from-cyan-500 to-blue-500 shadow-cyan-500/50"
                    )}
                  >
                    {area.badge}
                  </div>
                ...
                <div className={cn(
                  ...
                  isActive 
                    ...
                    : "bg-white/10 group-hover/key:bg-white/30"
                )} />
              </button>
            );
          })}
      ...
      <div className="relative border-b border-primary/20 bg-gradient-to-r from-card/90 via-card/70 to-card/90 backdrop-blur-2xl sticky top-0 z-20">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            ...
            {/* Quick Stats - Futuristic (ESQUERDA) */}
            <div className="flex items-center gap-3">
              {/* Tempo sugerido */}
