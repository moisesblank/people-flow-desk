...
export function MFAVerificationModal({
  ...
  const handleSendCode = useCallback(async () => {
    ...
    try {
      ...
      console.error('[MFAModal] Erro ao enviar código:', err);
  ...
  const handleVerifyCode = useCallback(async () => {
    ...
    if (verifyingRef.current) {
      console.log('[MFAModal] Verificação já em andamento, ignorando...');
      return;
    }
    
    // 🛡️ PROTEÇÃO: Não verificar o mesmo código duas vezes
    if (lastVerifiedCodeRef.current === code) {
      console.log('[MFAModal] Código já foi verificado anteriormente, ignorando...');
      return;
    }
    ...
    try {
      ...
      console.error('[MFAModal] Erro ao verificar código:', err);
