...
function useSidebar() {
  ...
  if (!context) {
    ...
    // This prevents crashes when components render before context is ready
    return {
...
const SidebarInset = React.forwardRef<HTMLDivElement, React.ComponentProps<"main">>(({ className, ...props }, ref) => {
  return (
    <main
      ref={ref}
      className={cn(
        // 🛡️ LAYOUT RULE: nunca travar altura por viewport — deixa o flex determinar
        // min-h-0 é obrigatório para permitir scroll no filho (MainContent)
        // overflow-x-hidden impede qualquer scrollbar horizontal global
        "relative flex min-h-0 flex-1 flex-col bg-background overflow-x-hidden",
...
// Larguras pré-definidas para skeletons (evita Math.random() em useMemo)
const SKELETON_WIDTHS = ['50%', '58%', '65%', '72%', '80%', '87%'] as const;
...
>(({ className, showIcon = false, ...props }, ref) => {
  // Largura determinística usando índice circular (evita anti-pattern Math.random em useMemo)
