...
// DOGMAS I-VIII - Performance Absoluta
// ============================================

export const PERFORMANCE_DOGMAS = {
  MAX_FIRST_PAINT: 100, MAX_INTERACTIVE: 300, MAX_API_RESPONSE: 200,
  CACHE_STALE_TIME: 30 * 1000, CACHE_GC_TIME: 15 * 60 * 1000,
  PREFETCH_DISTANCE: 500, BATCH_DELAY: 16, OVERSCAN_COUNT: 3,
  MAX_QUERY_TIME: 50, MAX_BUNDLE_SIZE: 150 * 1024,
} as const;
...
export function detectPerformanceTier(): PerformanceTier {
  if (typeof window === 'undefined') return { tier: 'mortal', score: 50, recommendations: [] };
  let score = 100;
  ...
  if (connection?.effectiveType === '2g' || connection?.effectiveType === 'slow-2g') {
    score -= 35; recommendations.push('Modo economia de dados');
  }
    ...
    this.observer = new IntersectionObserver((entries) => {
      for (const entry of entries) {
        if (entry.isIntersecting) {
          ...
          if (link.href && !this.prefetched.has(link.href)) this.prefetch(link.href);
        }
      }
...
class PerformanceMonitor {
  ...
  logReport(): void { console.log(`[EVANGELHO] Performance Score: ${this.getPerformanceScore()}`); }
}
...
if (typeof window !== 'undefined') {
  ...
  } else { prefetcher.init(); }
}

console.log('[EVANGELHO] ⚡ Dogmas I-VIII inicializados');
